...
function CreateInvoice() {
  ...
  const createMainInvoice = async (data: IEdcInvoiceForm) => {
    ...
    const res: ICreateResponse =
      await EdcServies.getInstance().createInvoiceMain(data, () => {
        setMainSubmitLoading(false);
      });
    if (res.IsSuccess) {
      toast.success(dictionary.en.successTxt, toastOptions);
      navigate(`/edc/view-invoice/${res?.Data?.id}`);
    }
    setMainSubmitLoading(false);
  };

  const createDraftInvoice = async (data: IEdcInvoiceForm) => {
    setDraftSubmitLoading(true);

    const res: ICreateResponse =
      await EdcServies.getInstance().createInvoiceDraft(data, () => {
        setDraftSubmitLoading(false);
      });
    if (res.IsSuccess) {
      toast.success(dictionary.en.successTxt, toastOptions);
      navigate(`/edc/view-invoice/draft/${res?.Data?.id}`);
    }
    setDraftSubmitLoading(false);
  };

  const onSubmit: SubmitHandler<IEdcInvoiceForm> = async (
            ...
            <Space>
              <Breadcrumb
                items={[
                  ...
                  {
                    title: dictionary.en.edcCreateInvoice
                  }
                ]}
              />
            </Space>
